...
function showView(name){$$('.view').forEach(v=>v.classList.remove('on'));const view=$('#view-'+(name||'home'));if(view) view.classList.add('on');$$('nav [data-view]').forEach(b=>b.classList.toggle('on',b.dataset.view===name));const bar=$('#topBar');if(bar) bar.classList.remove('hidden');if(name==='book'){const fr=$('#bookFrame');if(fr) fr.src='activity.html?v=13';}}
